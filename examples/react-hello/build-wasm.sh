#!/usr/bin/env bash
# Build the wasm bundle from `palimpsest-client-js` into ./pkg
# (gitignored). The output is a `wasm-bindgen --target web` build that
# `vite-plugin-wasm` understands.
set -euo pipefail
...
# wasm-bindgen-cli must be installed (`cargo install wasm-bindgen-cli`).
