...
use palimpsest_dataflow::collection::AsCollection;
use palimpsest_dataflow::input::InputSession;
use palimpsest_dataflow::operators::arrange::{ArrangeByKey, ArrangeBySelf};
use palimpsest_dataflow::operators::reduce::Reduce;
use palimpsest_dataflow::trace::TraceReader;

type Result = std::sync::mpsc::Receiver<
    timely::dataflow::operators::capture::Event<usize, Vec<((u64, i64), usize, i64)>>,
>;
...
fn test_import_vanilla() {
    run_test(
        |input_epochs| {
            timely::execute(timely::Config::process(4), move |worker| {
                ...
                for (t, changes) in input_epochs.into_iter().enumerate() {
                    ...
                    let &time = input.time();
                    for &((src, dst), w) in changes
                        .into_iter()
                        .filter(|&&((src, _), _)| (src as usize) % peers == index)
                    {
        ...
        vec![
            (0, vec![((1, 2), 1), ((2, 1), 1), ((4, 1), 1)]),
            (1, vec![((1, 1), 1), ((1, 2), -1), ((2, 1), -1)]),
            (2, vec![((1, 1), -1), ((2, 1), 1)]),
            (3, vec![((1, 2), 1), ((2, 1), -1), ((4, 1), -1)]),
            (4, vec![((1, 1), 1), ((1, 2), -1), ((2, 1), 1), ((4, 1), 1)]),
            (5, vec![((1, 1), -1), ((1, 2), 1), ((4, 1), -1)]),
        ],
    );
}
...
fn test_import_stalled_dataflow() {
    ...
    timely::execute(timely::Config::thread(), move |worker| {
        ...
        let (mut trace, probe1) = worker.dataflow(|scope| {
            let arranged = input.to_collection(scope).arrange_by_self();
        ...
        let probe2 = worker.dataflow(|scope| trace.import(scope).stream.probe());
