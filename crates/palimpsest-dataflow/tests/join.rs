...
use palimpsest_dataflow::operators::{Count, Join};
use palimpsest_dataflow::AsCollection;
use timely::dataflow::operators::capture::Extract;
use timely::dataflow::operators::{Capture, Map, ToStream};

#[test]
fn join() {
    let data = timely::example(|scope| {
        let col1 = vec![
            ((0, 0), Default::default(), 1),
            ((1, 2), Default::default(), 1),
        ]
        .into_iter()
        .to_stream(scope)
        .as_collection();
    ...
    assert_eq!(
        extracted[0].1,
        vec![
            ((0, (0, 'a')), Default::default(), 1),
            ((1, (2, 'B')), Default::default(), 1)
        ]
    );
}
...
fn join_map() {
    let data = timely::example(|scope| {
        ...
        col1.join_map(&col2, |k, v1, v2| (*k + *v1, *v2))
            .inner
            .capture()
    });
...
fn semijoin() {
    let data = timely::example(|scope| {
        ...
        .to_stream(scope)
        .as_collection();
        let col2 = vec![(0, Default::default(), 1)]
            .into_iter()
            .to_stream(scope)
            .as_collection();
...
#[test]
fn join_scale_100000() {
    join_scaling(100000);
}
#[test]
fn join_scale_1000000() {
    join_scaling(1000000);
}
