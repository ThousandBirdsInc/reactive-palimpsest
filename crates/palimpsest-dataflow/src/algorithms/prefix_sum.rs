...
pub fn aggregate<G, K, D, F>(
    collection: VecCollection<G, ((usize, K), D)>,
    combine: F,
) -> VecCollection<G, ((usize, usize, K), D)>
where
    G: Scope<Timestamp: Lattice>,
    K: ExchangeData + ::std::hash::Hash,
    D: ExchangeData + ::std::hash::Hash,
