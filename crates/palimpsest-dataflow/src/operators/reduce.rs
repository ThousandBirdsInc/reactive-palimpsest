...
use crate::operators::arrange::{ArrangeByKey, ArrangeBySelf, Arranged, TraceAgent};
use crate::trace::cursor::CursorList;
use crate::trace::implementations::containers::BatchContainer;
use crate::trace::implementations::merge_batcher::container::MergerChunk;
use crate::trace::implementations::{KeyBuilder, KeySpine, ValBuilder, ValSpine};
use crate::trace::TraceReader;
use crate::trace::{BatchReader, Builder, Cursor, Description, ExertionLogic, Trace};
...
pub trait Reduce<G: Scope<Timestamp: Lattice + Ord>, K: Data, V: Data, R: Semigroup> {
    ...
    /// use palimpsest_dataflow::operators::Reduce;
    ///
...
where
    ...
    T1: for<'a> TraceReader<Key<'a> = &'a K, KeyOwn = K, Val<'a> = &'a V, Diff = R>
        + Clone
        + 'static,
{
    fn reduce_named<L, V2: Data, R2: Ord + Abelian + 'static>(
...
pub trait Threshold<G: Scope<Timestamp: Lattice + Ord>, K: Data, R1: Semigroup> {
    ...
    /// ```
    /// use palimpsest_dataflow::input::Input;
    /// use palimpsest_dataflow::operators::Threshold;
    ///
    /// ::timely::example(|scope| {
...
impl<
    ...
    > Threshold<G, K, R1> for VecCollection<G, K, R1>
{
    fn threshold_named<R2: Ord + Abelian + 'static, F: FnMut(&K, &R1) -> R2 + 'static>(
        &self,
        name: &str,
...
where
    G: Scope<Timestamp = T1::Time>,
    T1: for<'a> TraceReader<Key<'a> = &'a K, KeyOwn = K, Val<'a> = &'a (), Diff = R1>
        + Clone
        + 'static,
{
    fn threshold_named<R2: Ord + Abelian + 'static, F: FnMut(&K, &R1) -> R2 + 'static>(
        ...
        self.reduce_abelian::<_, KeyBuilder<K, G::Timestamp, R2>, KeySpine<K, G::Timestamp, R2>>(
            name,
...
pub trait Count<G: Scope<Timestamp: Lattice + Ord>, K: Data, R: Semigroup> {
    ...
    /// ```
    /// use palimpsest_dataflow::input::Input;
    /// use palimpsest_dataflow::operators::Count;
    ///
    /// ::timely::example(|scope| {
...
impl<
        ...
        K: ExchangeData + Hashable,
        R: ExchangeData + Semigroup,
    > Count<G, K, R> for VecCollection<G, K, R>
{
    fn count_core<R2: Ord + Abelian + From<i8> + 'static>(&self) -> VecCollection<G, (K, R), R2> {
...
where
    G: Scope<Timestamp = T1::Time>,
    T1: for<'a> TraceReader<Key<'a> = &'a K, KeyOwn = K, Val<'a> = &'a (), Diff = R>
        + Clone
        + 'static,
{
    fn count_core<R2: Ord + Abelian + From<i8> + 'static>(&self) -> VecCollection<G, (K, R), R2> {
...
pub trait ReduceCore<G: Scope<Timestamp: Lattice + Ord>, K: ToOwned + ?Sized, V: Data, R: Semigroup>
{
    ...
    /// ```
    /// use palimpsest_dataflow::input::Input;
    /// use palimpsest_dataflow::operators::reduce::ReduceCore;
    /// use palimpsest_dataflow::trace::Trace;
    /// use palimpsest_dataflow::trace::implementations::{ValBuilder, ValSpine};
    ///
    /// ::timely::example(|scope| {
    ///
    ///     let trace =
    ...
    where
        ...
        Bu: Builder<
            Time = T2::Time,
            Input = Vec<((K::Owned, V), T2::Time, T2::Diff)>,
            Output = T2::Batch,
        >,
        ...
        V: Clone + Ord,
    {
            ...
            ),
        {
            ...
            let mut batch_replay =
                self.batch_history
                    .replay_key(batch_cursor, batch_storage, key, |time| {
                        C3::owned_time(time)
                    });
            ...
            let mut input_replay = if let Some(meet) = meet.as_ref() {
                self.input_history
                    .replay_key(source_cursor, source_storage, key, |time| {
            ...
            let mut output_replay = if let Some(meet) = meet.as_ref() {
                self.output_history
                    .replay_key(output_cursor, output_storage, key, |time| {
                ...
                if !upper_limit.less_equal(&next_time) {
                    ...
                    if interesting {
                        ...
                        for &((value, ref time), ref diff) in output_replay.buffer().iter() {
                            if time.less_equal(&next_time) {
                                self.output_buffer
                                    .push((C2::owned_val(value), diff.clone()));
                            } else {
                                self.temporary.push(next_time.join(time));
                            }
                        }
                        ...
                        if !self.update_buffer.is_empty() {
                            ...
                            let idx = outputs
                                .iter()
                                .rev()
                                .position(|(time, _)| time.less_equal(&next_time));
