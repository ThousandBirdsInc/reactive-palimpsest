...
use crate::operators::arrange::{ArrangeByKey, ArrangeBySelf, Arranged};
...
pub trait Join<G: Scope, K: Data, V: Data, R: Semigroup> {
    ...
    /// use palimpsest_dataflow::operators::Join;
    ///
    ...
    fn join<V2, R2>(
        &self,
        other: &VecCollection<G, (K, V2), R2>,
    ) -> VecCollection<G, (K, (V, V2)), <R as Multiply<R2>>::Output>
    where
        K: ExchangeData,
        V2: ExchangeData,
    ...
    fn semijoin<R2>(
        &self,
        other: &VecCollection<G, K, R2>,
    ) -> VecCollection<G, (K, V), <R as Multiply<R2>>::Output>
    where
        K: ExchangeData,
        R2: ExchangeData + Semigroup,
        R: Multiply<R2, Output: Semigroup + 'static>;
...
{
    fn join_map<V2: ExchangeData, R2: ExchangeData + Semigroup, D: Data, L>(
        &self,
        other: &VecCollection<G, (K, V2), R2>,
        mut logic: L,
    ) -> VecCollection<G, D, <R as Multiply<R2>>::Output>
    ...
    fn semijoin<R2: ExchangeData + Semigroup>(
        &self,
        other: &VecCollection<G, K, R2>,
    ) -> VecCollection<G, (K, V), <R as Multiply<R2>>::Output>
    where
    ...
    fn antijoin<R2: ExchangeData + Semigroup>(
        &self,
        other: &VecCollection<G, K, R2>,
    ) -> VecCollection<G, (K, V), R>
    where
        R: Multiply<R2, Output = R>,
        R: Abelian + 'static,
    {
    ...
    V: Data + 'static,
{
    fn join_map<V2: ExchangeData, R2: ExchangeData + Semigroup, D: Data, L>(
        ...
        other: &VecCollection<G, (K, V2), R2>,
        mut logic: L,
    ) -> VecCollection<G, D, <Tr::Diff as Multiply<R2>>::Output>
    where
    ...
    fn semijoin<R2: ExchangeData + Semigroup>(
        &self,
        other: &VecCollection<G, K, R2>,
    ) -> VecCollection<G, (K, V), <Tr::Diff as Multiply<R2>>::Output>
    where
        Tr::Diff: Multiply<R2, Output: Semigroup + 'static>,
    {
        let arranged2 = other.arrange_by_self();
    ...
    fn antijoin<R2: ExchangeData + Semigroup>(
        &self,
        other: &VecCollection<G, K, R2>,
    ) -> VecCollection<G, (K, V), Tr::Diff>
    where
        Tr::Diff: Multiply<R2, Output = Tr::Diff>,
        Tr::Diff: Abelian + 'static,
    {
...
>
{
    ...
    /// ```
    /// use palimpsest_dataflow::input::Input;
    /// use palimpsest_dataflow::operators::arrange::ArrangeByKey;
    /// use palimpsest_dataflow::operators::join::JoinCore;
    /// use palimpsest_dataflow::trace::Trace;
    ///
    /// ::timely::example(|scope| {
    ///
    ...
    fn join_core<Tr2, I, L>(
        ...
        result: L,
    ) -> VecCollection<G, I::Item, <R as Multiply<Tr2::Diff>>::Output>
    where
    ...
    R: ExchangeData + Semigroup,
{
    ...
    where
        ...
        L: FnMut(&K, &V, Tr2::Val<'_>) -> I + 'static,
    {
        self.arrange_by_key().join_core(stream2, result)
    }
    ...
    {
        self.arrange_by_key()
            .join_core_internal_unsafe(stream2, result)
    }
}
...
impl<CB: ContainerBuilder> timely::container::ContainerBuilder for EffortBuilder<CB> {
    ...
    fn extract(&mut self) -> Option<&mut Self::Container> {
        ...
        self.0
            .replace(self.0.take() + extracted.as_ref().map_or(0, |e| e.record_count() as usize));
        extracted
    }
    ...
    fn finish(&mut self) -> Option<&mut Self::Container> {
        ...
        self.0
            .replace(self.0.take() + finished.as_ref().map_or(0, |e| e.record_count() as usize));
        finished
    }
}
...
pub fn join_traces<G, T1, T2, L, CB>(
...
where
    ...
    L: FnMut(
            ...
            &T1::Diff,
            &T2::Diff,
            &mut JoinSession<T1::Time, CB, Capability<T1::Time>>,
        ) + 'static,
    CB: ContainerBuilder,
{
    ...
    T: Timestamp + Lattice + Ord,
{
    ...
    #[inline(never)]
    fn work<L, CB: ContainerBuilder>(
        &mut self,
        output: &mut OutputBuilderSession<T, EffortBuilder<CB>>,
        mut logic: L,
        fuel: &mut usize,
    ) where
        L: for<'a> FnMut(
            ...
            &C1::Diff,
            &C2::Diff,
            &mut JoinSession<T, CB, Capability<T>>,
        ),
    {
        ...
        while let (Some(batch_key), Some(trace_key), true) = (
            batch.get_key(batch_storage),
            trace.get_key(trace_storage),
            effort < *fuel,
        ) {
            match trace_key.cmp(&batch_key) {
                ...
                Ordering::Equal => {
                    ...
                    thinker
                        .history2
                        .edits
                        .load(batch, batch_storage, |time| C2::owned_time(time));
    ...
    C2: Cursor<Time = C1::Time>,
{
    ...
    fn think<F: FnMut(C1::Val<'a>, C2::Val<'a>, C1::Time, &C1::Diff, &C2::Diff)>(
        &mut self,
        mut results: F,
    ) {
