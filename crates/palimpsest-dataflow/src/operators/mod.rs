...
pub use self::reduce::{Count, Reduce, Threshold};
        ...
        L: Fn(C::TimeGat<'_>) -> C::Time,
    {
        ...
        while let Some(val) = cursor.get_val(storage) {
            cursor.map_times(storage, |time1, diff1| {
                self.push(logic(time1), C::owned_diff(diff1))
            });
            self.seal(val);
    ...
    fn replay<'history>(&'history mut self) -> HistoryReplay<'storage, 'history, C> {
        ...
        for value_index in 0..self.edits.values.len() {
            ...
            for edit_index in lower..upper {
                ...
                self.history
                    .push((time.clone(), time, value_index, edit_index));
            }
        }
