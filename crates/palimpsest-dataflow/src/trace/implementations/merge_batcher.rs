...
pub use container::{ColMerger, VecMerger};
...
pub mod container {
    ...
    where
        for<'a> MC: MergerChunk<TimeOwned: Ord + PartialOrder + Data>
            + Clone
            + PushInto<<MC as DrainContainer>::Item<'a>>
            + 'static,
    ...
    pub mod vec {
        ...
        pub type VecMerger<D, T, R> =
            super::ContainerMerger<Vec<(D, T, R)>, std::collections::VecDeque<(D, T, R)>>;
    ...
    pub mod columnation {
        ...
        pub type ColMerger<D, T, R> =
            super::ContainerMerger<TimelyStack<(D, T, R)>, TimelyStackQueue<(D, T, R)>>;
        ...
        impl<D: Ord + Columnation, T: Ord + Columnation, R: Columnation>
            ContainerQueue<TimelyStack<(D, T, R)>> for TimelyStackQueue<(D, T, R)>
        {
        ...
        impl<
                ...
                T: Ord + timely::PartialOrder + Clone + Columnation + 'static,
                R: Default + Semigroup + Columnation + 'static,
            > MergerChunk for TimelyStack<(D, T, R)>
        {
            type TimeOwned = T;
            type DiffOwned = R;
