...
pub type ColValBatcher<K, V, T, R> =
...
pub type OrdKeyBatcher<K, T, R> =
    MergeBatcher<Vec<((K, ()), T, R)>, VecChunker<((K, ()), T, R)>, VecMerger<(K, ()), T, R>>;
/// A builder for ordered lists.
pub type RcOrdKeyBuilder<K, T, R> =
...
/// A batcher for columnar storage
pub type ColKeyBatcher<K, T, R> = MergeBatcher<
    Vec<((K, ()), T, R)>,
    ColumnationChunker<((K, ()), T, R)>,
    ColMerger<(K, ()), T, R>,
>;
/// A builder for columnar storage
...
pub mod layers {
    ...
    impl<O: for<'a> BatchContainer<ReadItem<'a> = usize>, T: BatchContainer, D: BatchContainer>
        Default for Upds<O, T, D>
    {
        fn default() -> Self {
            Self::with_capacity(0, 0)
        }
    ...
    impl<T: BatchContainer, D: BatchContainer> Default for UpdsBuilder<T, D> {
        fn default() -> Self {
            Self {
                ...
                diff_con: BatchContainer::with_capacity(1),
            }
        }
    }
        ...
        D: BatchContainer<Owned: crate::difference::Semigroup>,
    {
        ...
        pub fn seal<O: for<'a> BatchContainer<ReadItem<'a> = usize>>(
            &mut self,
            upds: &mut Upds<O, T, D>,
        ) -> bool {
            ...
            if self.stash.len() == 1 {
                ...
                self.time_con.clear();
                self.time_con.push_own(time);
                self.diff_con.clear();
                self.diff_con.push_own(diff);
                if upds.times.last() == self.time_con.get(0)
                    && upds.diffs.last() == self.diff_con.get(0)
                {
                    self.total += 1;
                    self.stash.clear();
...
pub mod val_batch {
    ...
    use super::{layers::UpdsBuilder, Layout, Upds, Vals};
                ...
                .cmp(&source2.keys.index(self.key_cursor2))
            {
                ...
                Ordering::Equal => {
                        ...
                        self.merge_vals((source1, lower1, upper1), (source2, lower2, upper2))
                    {
                        self.result
                            .keys
                            .push_ref(source1.keys.index(self.key_cursor1));
        ...
        fn merge_vals(
            ...
            while lower1 < upper1 && lower2 < upper2 {
                ...
                match source1
                    .vals
                    .get_abs(lower1)
                    .cmp(&source2.vals.get_abs(lower2))
                {
                    Ordering::Less => {
    ...
    impl<L: Layout> Cursor for OrdValCursor<L> {
        ...
        fn val<'a>(&self, storage: &'a OrdValBatch<L>) -> Self::Val<'a> {
            storage.storage.vals.get_abs(self.val_cursor)
        }
        ...
        fn seek_key(&mut self, storage: &OrdValBatch<L>, key: Self::Key<'_>) {
            self.key_cursor +=
                storage
                    .storage
                    .keys
                    .advance(self.key_cursor, storage.storage.keys.len(), |x| {
                        <L::KeyContainer as BatchContainer>::reborrow(x)
        ...
        fn seek_val(&mut self, storage: &OrdValBatch<L>, val: Self::Val<'_>) {
            self.val_cursor += storage.storage.vals.vals.advance(
                self.val_cursor,
                storage.storage.vals.bounds(self.key_cursor).1,
                |x| {
    ...
    where
        L: for<'a> Layout<KeyContainer: PushInto<CI::Key<'a>>, ValContainer: PushInto<CI::Val<'a>>>,
...
pub mod key_batch {
    ...
    use super::{layers::UpdsBuilder, Layout, Upds};
    ...
    impl<L: Layout<ValContainer: BatchContainer<Owned: Default>>> Merger<OrdKeyBatch<L>>
        for OrdKeyMerger<L>
    where
