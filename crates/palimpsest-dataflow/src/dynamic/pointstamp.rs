...
#[derive(
    Hash, Default, Clone, Eq, PartialEq, Ord, PartialOrd, Debug, Serialize, Deserialize, Columnar,
)]
...
mod columnation {
    ...
    pub struct PointStampStack<R: Region<Item: Columnation>>(
        <Vec<R::Item> as Columnation>::InnerRegion,
    );
    ...
    impl<R: Region<Item: Columnation>> Region for PointStampStack<R> {
        ...
        unsafe fn copy(&mut self, item: &Self::Item) -> Self::Item {
            Self::Item {
                vector: self.0.copy(&item.vector),
            }
        }
